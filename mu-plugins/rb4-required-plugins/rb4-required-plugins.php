 ...
 * @subpackage Project
 */
class RB4_Required_Plugins {

	/**
	 * Instance of this class.
	 *
	 * @var rb4_Required_Plugins object
	 */
	 ...
	 * @since  0.1.0
	 * @return rb4_Required_Plugins A single instance of this class.
	 */
	...
	public function activate_if_not() {
		foreach ( $this->get_required_plugins() as $plugin ) {
			if ( ! is_plugin_active( $plugin ) ) {
				...
				activate_plugin( $plugin, null, apply_filters( 'rb4_required_plugin_network_activate', is_multisite(), $plugin ) );
			}
		}
	...
	public function required_text_markup() {
		$this->required_text = apply_filters( 'rb4_required_plugins_text', sprintf( '<span style="color: #888; font-style: italic;">%s</span>', __( 'RB4 Required Plugin', 'rb4-required-plugins' ) ) );
	}
	...
	public function filter_plugin_links( $actions = array(), $plugin ) {
		...
		if ( array_key_exists( 'deactivate', $actions ) && in_array( $plugin, $required_plugins ) ) {
			...
			if ( ! is_multisite() || apply_filters( 'rb4_required_plugin_network_activate', true, $plugin ) ) {
	...
	/**
	 * Get the plugins that are required for the project. Plugins will be registered by the rb4_required_plugins filter
	 *
	 * @since  0.1.0
	 *
	 * @return array
	...
	public function get_required_plugins() {
		return (array) apply_filters( 'rb4_required_plugins', array() );
	}

	/**
	 * Get the network plugins that are required for the project. Plugins will be registered by the rb4_network_required_plugins filter
	 *
	 * @since  0.1.3
	 *
	 * @return array
	...
	public function get_network_required_plugins() {
		return (array) apply_filters( 'rb4_network_required_plugins', array() );
	}
	...
	public function l10n() {
		...
		$loaded = load_plugin_textdomain( 'rb4-required-plugins', false, '/languages/' );
		if ( ! $loaded ) {
			$loaded = load_muplugin_textdomain( 'rb4-required-plugins', '/languages/' );
		}
		if ( ! $loaded ) {
			$loaded = load_theme_textdomain( 'rb4-required-plugins', '/languages/' );
		}

		if ( ! $loaded ) {
			$locale = apply_filters( 'plugin_locale', get_locale(), 'rb4-required-plugins' );
			$mofile = dirname( __FILE__ ) . '/languages/rb4-required-plugins-'. $locale .'.mo';
			load_textdomain( 'rb4-required-plugins', $mofile );
		}
	}

}

rb4_Required_Plugins::init();
